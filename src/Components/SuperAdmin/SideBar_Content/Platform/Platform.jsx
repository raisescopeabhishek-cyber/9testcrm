...
export default function Platform() {
  ...
  const addField = async () => {
    ...
    if (newField.name) {
      try {
        const res = await backendApi.post(`/api/auth/add-platform`, {
          name: newField.name,
          status: "active",
        });
        if (res.data.status) {
  ...
  const toggleActive = async (id, currentStatus) => {
    ...
    try {
      const res = await backendApi.put(`/api/auth/update-platform`, {
        id,
        status: newStatus,
      });
      if (res.data.status) {
  ...
  const deletePlatform = async (id) => {
    const toastId = toast.loading("Please wait...");
    try {
      const res = await backendApi.delete(`/api/auth/delete-platform/?id=${id}`);
      if (res.data.status) {
  ...
  const getAllPlatforms = async () => {
    ...
    setError(null);
    try {
      const res = await backendApi.get(`/api/auth/get-platforms`);
